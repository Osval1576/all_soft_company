...
---
# H1 · Nucleo Multi-tenant (rama feat/multitenant-h1, iniciada 2026-07-12)
Plan: docs/superpowers/plans/2026-07-12-multitenant-h1.md
Base de rama: 83ea513c
H1-T1: complete (commit f59c15b2, review Approved; 1/1 + check limpio). Middleware fail-closed razonado adversarialmente (AnonymousUser safe, exempts correctos vs urls reales, superuser-con-org-suspendida bloqueado = deliberado). Minors forward: test de uppercase de slug en T2; middleware va DESPUES de AuthenticationMiddleware en T3.
H1-T2: complete (commits 90a2b6df + docstring 76a062e8, review Approved; tenancy+users 12/12, dev migrado 9 users + 28 tickets -> ALS). Important resuelto: docstring de seed_org (barrido total, operador de plataforma post-migrate con org=None). Minors: import mid-file (convencion local), _DEFAULT_POLICIES inerte hasta T4.
H1-T3: complete (commit ca801fbb, review opus Approved; GATE SUITE COMPLETA FRESCA 168/168). HALLAZGO CRITICO del implementer: el middleware del plan era FAIL-OPEN (la API autentica por JWT-cookie, request.user anonimo en middleware) -> adaptado con _api_user() reusando CookieJWTAuthentication; reviewer verifico: sin bypass (unico authenticator, cookie-only, sin Bearer), _force_auth_user no es vector externo, excepciones fail-closed a 401, org/user siempre coherentes. Fixtures multi-org en 7 suites sin debilitar asserts. Forward: WS scoping en T5/T6; comentario de advertencia si algun dia se suma SessionAuthentication.
H1-T4: complete (commits 6b056247 + fix 1ac097f4, review Needs fixes -> 3 Important resueltos; sla+metrics 60/60 post-fix, 103/103 pre-fix en 4 apps). Findings reales del reviewer: organization writable en SlaConfigSerializer, scheduler_enabled por-org decorativo, 500 superuser org=None. Adaptaciones aprobadas: scheduler dev min(interval), memoizacion calendarios por org.
H1-T5: complete (commits 20c87762 + fix 88ddc07d, review Needs fixes -> 1 Important resuelto; tickets_t 36/36 + TenantScopingTests 5/5). Finding real: 500 en POST de superuser sin org -> guard 400 en TicketCreateSerializer + test. Verificado por reviewer: messages/events heredan 404 via get_object, take TOCTOU intacto sobre queryset scoped, consumer WS sin atajos, referencia 28<=32 chars. Minors: lock ampliado a sla/csat en take (MySQL ok), value.is_superuser en validate del tecnico (pre-existente, cerrado en la practica por el org-check).
H1-T6: complete (commit a8f91e09, review Approved; notifications+metrics+users 53/53). Verificado: todos los kinds de notif con org (los directos same-org por construccion — enforced, no asumido), 21 callers de windowed_tickets migrados, anti-N+1 intacto sin re-baseline, UserCreateSerializer sin organization writable (sin smuggling), WS por user_id sin broadcast de org. Minor para review final: perform_create de users sin guard org=None (huerfano silencioso, patron ya aceptado en HolidayViewSet).
H1-T8: complete (commit f5bc121b; build limpio). Controller inline: org-name chip en TopBar (mono, uppercase, hairline separator), v-if para plataforma sin org. Verificacion visual en la pasada final de H1.
H1-T7: complete (commit 23b2c4d1, adversarial 11/11 + guard + NOT NULL; tenancy+landing_cms 52/52). Guard destapo 2 querysets crudos: csat/views y landing_cms/contact_view -> refactorizados via org_tickets (contacto publico -> ALS via _als_org). 2 tests rotos por NOT NULL arreglados (creaban tickets sin org; garantia ahora estructural). Allowlist minimo: scoping.py + serializer de referencia.
REVIEW FINAL H1 (opus): READY TO MERGE, sin blockers, aislamiento coherente HTTP+WS end-to-end. Gate completo fresco 196/196. Fix-wave post-review (f33eb71f): usuario de contacto anonimo -> org ALS; PoliciesView.get guard org None -> 404 (consistencia); guard CI excluye .venv/site-packages (perf+robustez); WS test asserta close_code 4403. Suites tocadas 90/90. Minor documentado no-arreglado: UserCreateSerializer expone is_staff writable (same-org, sin escalada a superuser, django-admin ya restringido en G) -> decision de producto, follow-up H2.


---
# H2 Registro + Onboarding (rama feat/onboarding-h2, iniciada 2026-07-13)
Plan: docs/superpowers/plans/2026-07-13-registro-onboarding-h2.md
Base de rama: c1be9ee3
H2-T1: en progreso. DEFECTO DE PLAN detectado por el implementer (buen catch): MySQL no soporta UniqueConstraint condicional (W036 -> lo omite), asi que la unicidad de invitacion PENDIENTE por (org,email) no puede ser DB-level. Resolucion: sacar el constraint (no-op en MySQL) + enforcar en el serializer de T3 (validate_email ya lo hace) + ajustar el test de T1 a solo model-basics. No requiere adjudicacion del usuario (correccion factica que preserva la intencion del spec).
H2-T1: complete (commit 2b84fa65, review Approved; accounts 2/2, sin W036). Constraint condicional removido (MySQL), defaults callables, is_valid/is_acceptable correctos, emails fail_silently=False. RECORDATORIO T3: el reviewer marca que la unicidad de invitacion pendiente por (org,email) queda SIN guard DB hasta que T3 agregue el check en InvitationCreateSerializer.validate_email -> verificar que ese check exista y este testeado en T3.
H2-T2: complete (commits 80648324 + fix f0608040, review Needs fixes -> 3 Important resueltos; accounts 7/7). Findings reales (superficie publica): (1+3) email SMTP sincrono era canal lateral de anti-enumeracion en resend (timing + 500-vs-200) y estrandaba cuentas en register -> movido a background (patron NOTIFICATIONS_EMAIL_ASYNC de notifications, fail_silently); (2) TOCTOU de unicidad -> IntegrityError capturado -> 400. Minors documentados no-arreglados: verify sin throttle (uuid 128-bit, ok), password sin AUTH_PASSWORD_VALIDATORS (plan-mandated min_length=8).
H2-T3: complete (commit 6203fee2, review Approved; accounts 10/10). Anti-escalada verificada estructural (AcceptInvitationSerializer sin role/org; view usa inv.role/inv.organization; perform_create fuerza org/invited_by server-side; sin **validated_data). Unicidad de pendiente en validate_email (unico guard, DB removido en T1). Viewset scoped, destroy=revoked, accept loguea via cookies. Minors: (1) TOCTOU en accept concurrente -> 500 (narrow, sin impacto seguridad) -> FOLLOW-UP select_for_update; (2) T5 DEBE agregar tests de doble-pending-400 y cross-org-404 (diferidos aqui).
H2-T4: complete (commit 5b4a053b + fix 3743adc7, review Approved; users 12/12, accounts 10/10, tenancy 22/22). create retirado (http_method_names sin post), is_staff read-only, guard ultimo-admin (exclude pk propio, permite degradar si hay otro). FIX de seguridad REAL (finding pre-existente del reviewer): IsAdminOrSelf + role writable dejaba a un CUSTOMER auto-promoverse a ADMIN via PATCH a si mismo -> cerrado (no-admin no cambia role/is_active propios) + 3 tests. Follow-up documentado: UserCreateSerializer dead code.
H2-T5: complete (commits 7f2924c2 + ed40d5ec + fix 02872b01, review Approved; adversarial 10/10, accounts 20/20). 9 ataques reales verificados contra source (anti-escalada rol/org, cross-org 404 sin cambiar B, token expirado 410, reuso count==1, GET publico set exacto de 3 keys, doble-pending 400, AGENT+CUSTOMER 403 sin crear invitacion). Fix: accept maneja IntegrityError->409 (paridad con register; ambos reviewers T3+T5 lo flagearon) + test de carrera. Minor cosmetico: assert (400,410) sobre-permisivo en test de reuso. BACKEND H2 COMPLETO.
H2-T6: complete (commit fd634c39, review Approved; build limpio, verificado e2e contra backend). 3 vistas publicas (registro/verificar/invitacion) estilo LoginView, api client con endpoints verificados 1:1 contra urls/serializers, rutas meta.public no rebotan (guard first-check), accept -> loadMe + redirectByRole (reusa patron del store), payload de accept SIN role/org/email (anti-escalada front-side). Minor: carrera invitacion expira entre GET y POST (manejada). Link registro desde login.
H2-T7: complete (commit d5ba01c8, review Approved; build limpio). AdminMembers.vue: seccion miembros (rol select + activar/desactivar con REVERT-ON-ERROR verificado contra los 400 reales del guard) + invitaciones (invitar AGENT/CUSTOMER, revocar con confirm-2-clicks). API helpers verificados contra backend. Removio el modal muerto de alta-con-password de AdminDashboard (scope justificado, sin codigo colgado). Minors: onMounted sin catch (espeja AdminSla), createUser/deleteUser dead exports en tickets.api, ROLE_LABELS sin ADMIN (inalcanzable). H2 FRONTEND COMPLETO -> 7/7 tasks.

## H2: 7/7 tasks. Pendiente: review final de rama + gate fresco + merge.


---
# H3 Billing + Stripe (rama feat/billing-h3, iniciada 2026-07-13)
Plan: docs/superpowers/plans/2026-07-13-billing-h3.md
Base de rama: 427c887f
H3-T1: complete (commit cc62a273, review Approved; billing 3/3 + regresion accounts+tenancy 42/42, reviewer re-corrio 45/45). effective_plan preciso (Free solo si trial vencido), signal no colisiona con SLA, override a Business load-bearing verificado (seed crea Pro fuera de tx -> signal crea Pro-trial -> provision_test_org lo fuerza a Business para ~10 suites). Minors: effective_plan con trial_ends_at=None no degrada (inalcanzable hoy); seed reverse noop.
H3-T2: complete (commit 4919dac7, review Approved; billing AgentLimitTests 5/5, billing+accounts+users 40/40). services can_add_agent (CUSTOMER excluido, inactivos no cuentan, None=ilimitado, sin-sub=Free); hooks en InvitationCreateSerializer.validate y UserSerializer.validate coexisten con guards de H2-T4 (independientes, no se pisan); becomes_active_agent no dispara si ya-era-agente-activo ni al degradar. Fix real (folded): provision_test_org mutaba sub de query separada dejando el reverse OneToOne cacheado stale -> muta org.subscription directo (verificado por el reviewer). Minor: regresion full-suite la cubre el gate final.
H3-T3: complete (commit 9f02b144, review Approved; billing 12/12, stripe 15.3.1 pinneado). Gateway aislado (unico import stripe, grep confirmado), API verificada por introspeccion del paquete 15.3.1, 503 sin config + free 400, scoped por org, subscription usa effective_plan + agent_count/limit/plans, import estilo modulo mockeable. Minor: superuser org=None -> AttributeError en org.subscription (heredado del brief, patron del proyecto).
H3-T4: complete (commits 260997b2 + fix 4443a87d, review opus Needs fixes -> CRITICAL+IMPORTANT resueltos; billing 18/18, billing+tenancy 38/38). Mecanica de seguridad impecable (firma raw body, 400 sin procesar, exencion acotada con slash final, status solo por webhook). DOS BUGS REALES del snippet del plan corregidos: (CRITICAL) customer.subscription.updated hardcodeaba ACTIVE -> re-activaba impagos/cancelados y anulaba dunning -> ahora mapea el status real; (IMPORTANT) el plan comprado nunca se aplicaba (business dejaba plan=pro) -> reconcilia plan por price.id (created+updated). Idempotencia ahora atomica (create-first + IntegrityError=no-op) + test real (mock _handle_event 1 vez).
H3-T5: complete (commit a77528c6, review Approved; billing 21/21, YAML valido). expire_trials filtro correcto (status=trial AND vencido, no toca vigentes/pagas, idempotente, free-ausente->0), command patron check_sla (max-loops sin sleep final, KeyboardInterrupt limpio, excepcion logueada continua), servicio compose trial-scheduler copia limpia del scheduler. Sin issues.
H3-T6: complete (commits 010756c8 + fix d3441351, review Needs fixes -> resuelto; billing 25/25). Ataques 2/3/4 reales (no-admin checkout 403 con is_configured mockeado, webhook sin firma 400 antes de procesar, PATCH subscription 405 sin cambio). Ataque 1 era VACUO (A y B identicas via provision_test_org -> respuesta igual con o sin leak; assert comparaba IDs en DB no contenido HTTP) -> reescrito: A downgrade a free+1 agente, B business+3 agentes, asserta que la respuesta de A refleja SU estado y NO el de B. Codigo de scoping siempre fue correcto (request.organization).
H3-T7: complete (commit ced4800c, review Approved sin issues; build limpio). AdminSubscription.vue: contrato verificado campo por campo contra SubscriptionView, startCheckout {plan_key}->redirect window.location, 503 manejado (checkout+portal), trial days Math.ceil clamp, uso N/M con inf, tarjetas por order (Mejorar salvo free/actual), ?success->toast+refetch+limpieza query, ruta ADMIN + nav. Estetica v2 sin tokens inventados.
H3-T8: complete (commit 0e1645a9, review Approved; build limpio). AdminMembers muestra Agentes N/M (inf si null), bloqueo SOLO para rol AGENT al tope (inviteBlocked computed de newInv.role, CUSTOMER libre), aviso con link a /admin/suscripcion, reusa getSubscription, NO incrementa contador local (agent_count = activos backend), carrera 400 manejada. Aditivo, no rompe H2. Minors cosmeticos. H3 8/8 COMPLETO.

## H3: 8/8 tasks. Pendiente: review final + gate fresco + merge.
H3 REVIEW FINAL (opus): 1 CRITICAL merge-blocker que los reviews por-task se perdieron -> accept de invitacion (AcceptInvitationView) creaba AGENT activo SIN chequear can_add_agent (3er camino de alta, solo invitar+activar estaban gateados). Bypass: Free invita N (count 0 mientras nadie acepta) luego todos aceptan -> N sobre 2; o downgrade Business->Free. FIX 166fa863: check de limite en el accept (dentro del atomic, 409 si AGENT sobre tope) + 2 tests adversariales (accept sobre limite 409, batch-invite-luego-accept corta en el 3ro). accounts+billing 47/47. Resto del review READY (webhook unica fuente de verdad, aislamiento cross-org, gateway aislado, trial/migraciones/fixture-safety limpio). Minor: UserCreateSerializer dead code.

---
# H4 Branding por tenant (rama feat/branding-h4, iniciada 2026-07-19)
Plan: docs/superpowers/plans/2026-07-19-branding-h4.md
Base de rama: 0cb69d39
H4-T1: complete (commit c02d82a8, review Spec ✅ / Quality Approved; tenancy.tests_branding 3/3). Modelo OrganizationBranding (OneToOne org, display_name, accent_color hex-validado, logo ImageField, default_dark, updated_at) + HEX_COLOR_VALIDATOR modular + effective_display_name. Migracion 0003 solo CreateModel. Minors no bloqueantes: __str__ extra (benigno), sin test explicito de empty-string (no pedido en brief).
H4-T2: complete (commit bc681523 + fix 3da531ba, review Spec ✅ / Quality Needs-fixes -> resuelto; tenancy.tests_branding.BrandingAdminApiTests 8/8). API admin /api/branding/: BrandingSerializer (logo write_only + logo_url/effective_display_name read-only, validate_logo <=512KB + PNG/JPEG/WEBP), BrandingView GET (defaults sin persistir) / PUT (gateado 403 si no pago, scoped a request.organization, sin org por body). branding_enabled() + branding_payload() en serializers. NOTA: implementer murio por ENOTFOUND tras escribir archivos pre-commit; controller verifico vs brief + corrio tests + commiteo. FIX de review (2 Important): (a) PUT persistia fila fantasma en 400 (get_or_create antes de validar) -> getattr-or-unsaved, valida antes de tocar BD + test regresion; (b) validacion de logo sin tests (_png_bytes muerto) -> +2 tests (PNG ok, GIF 400). Minor diferido: superuser org=None (patron proyecto, igual que H3-T3).
H4-T3: complete (commit 75d350c8, review Spec ✅ / Quality Approved; tenancy.tests_branding.BrandingPublicApiTests 5/5). PublicBrandingView (AllowAny) GET /api/public/branding/<slug>/ -> branding_payload; 404 unificado para 3 casos (slug inexistente / inactiva / plan free) sin leak de existencia; lookup slug.upper(). /api/me/ gana clave branding (payload o None) sin romper claves previas. Sin cambios de middleware (anonimos ya pasan). Reviewer confirmo no colision de ruta con landing_cms.public_urls y que la respuesta tiene EXACTAMENTE los 4 campos. Minors: import local de Organization (patron del archivo).
H4-T4: complete (commit 2d9cf811, review Spec ✅ / Quality Approved; tenancy.tests_branding_adversarial 5/5, sin cambios de codigo de produccion -> aislamiento ya solido). 5 ataques NO vacuos (reviewer trazo cada uno contra prod): (a) PUT ignora organization en body (fields excluye organization) y escribe en org del user, verifica DB de A y B; (b) admin A no lee branding de B; (c) endpoint publico solo 4 campos (asserta contenido, no status); (d) org free -> branding 404 (downgrade via org.subscription cacheado); (e) CUSTOMER no edita (403 en IsAdmin). Orgs A/B en estados distinguibles (sentinels de color unicos).

## H4 BACKEND: 4/4 tasks (T1-T4). Pendiente frontend T5-T7 + emails opcional T8.
H4-T5: complete (commit 8c6b4b58 + fix d6a83069, review Spec ✅ / Quality Approved-with-fixes; npm run build limpio). useBranding.js (applyBranding sobrescribe --accent + 4 derivados via color-mix; clearBranding remueve exactamente esos 5; null/no-accent -> clearBranding sin crash), branding.api.js (getBranding/saveBranding/getPublicBranding), auth.store aplica branding tras loadMe, TopBar logo+display_name con fallback AS/AllSafe. FIX de review: (Important) loadMe catch no limpiaba branding -> colores stale de org previa si /api/me/ fallaba -> clearBranding() en catch; (Minor a11y) alt del logo -> brandName; (Minor def-in-depth) guard hex client-side en applyBranding. Solo tokens de acento tocados, neutros intactos.
KNOWN GAP (plan debt, para review final): default_dark se guarda/edita pero NUNCA se cablea a useTheme -> no aplica tema inicial a usuarios nuevos. El plan no lo cableo en ninguna tarea. Triage en review final: cablear (aplicar dark si default_dark y sin preferencia en localStorage) o documentar como no-implementado.
H4-T6: complete (commit cc4f2563, review Spec ✅ / Quality Approved sin issues; npm run build limpio). AdminBranding.vue en /admin/branding (meta role ADMIN): form nombre visible + color (input color + hex mismo v-model) + logo upload png/jpeg/webp con preview + toggle default_dark. Gating server-side: 403 de saveBranding -> estado upsell (link a /admin/suscripcion), nunca falso exito; extra getSubscription() on-mount para detectar free (fail-open, dentro de spec). Exito -> applyBranding(b) + auth.user.branding=b (TopBar reactivo sin reload). FormData (display_name/accent_color/default_dark/logo) verificado vs BrandingSerializer. Estetica hairline reusada.
H4-T7: complete (commit 535994a8 + fix 8d899080, review Spec ✅ / Quality Needs-fixes -> resuelto; npm run build limpio). OrgLoginView.vue (wrap LoginView, resuelve getPublicBranding(slug) on-mount, 404 -> fallback silencioso AllSafe), LoginView props brandName/brandLogo (default AllSafe/null, .brand-mark v-if -> ruta /login sin cambios), rutas o/:slug/login (org-login) + o/:slug/invitacion/:token (org-invitacion) meta public, AcceptInvitationView aplica branding por slug si presente. FIX de review (Important, bug cross-task real): onUnmounted(clearBranding) borraba el branding que loadMe YA habia aplicado tras login/accept -> dashboard con colores AllSafe hasta reload. Fix: onUnmounted limpia SOLO si !auth.user (logout-nav resetea a producto; login preserva branding de la org). Ambos casos verificados por trazado.

## H4: 7/7 tasks no-opcionales (T1-T7). Pendiente: review final de rama + gate fresco + decision T8 emails (opcional) + merge.

H4 REVIEW FINAL (opus): READY TO MERGE. Backend core verificado SOLIDO: aislamiento tenant (write scoped a request.organization, serializer sin campo organization -> body ignorado), superficie publica minima (solo 4 campos, 404 uniforme missing/inactive/free sin leak), gating de plan consistente en los 3 caminos (PUT/publico/me via branding_enabled -> effective_plan.key), trial-vencido->free propaga uniforme, fix 400-no-persiste confirmado, upload logo validado (512KB + Pillow format, SVG excluido), migracion 0003 limpia, applyBranding hex-guarded. 1 Important (bug cross-task): el guard !auth.user en onUnmounted fijaba branding de org AJENA en sesion logueada al visitar /o/OTRA/login -> FIX 241fb0a9: onUnmounted re-deriva (auth.user ? applyBranding(propio) : clearBranding) en ambas vistas + watch de slug en OrgLoginView. Minor #2 (slug-change) cerrado con el watch. #3 default_dark: gap diferido conocido. #4 runbook: nota de media_volume para logos agregada a docs/deploy.md.
H4 GATE: 270/270 tests en DB FRESCA (destruye keepdb + recrea, sin --keepdb), 1118s, exit 0, system check 0 issues. (H3 eran 249; +21 de H4: T1 3 + T2 8 + T3 5 + T4 5.) Rama feat/branding-h4 lista para merge. Pendiente SOLO: decision T8 emails (opcional) + como cerrar la rama.
