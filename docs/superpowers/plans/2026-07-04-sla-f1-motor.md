...
```
(El AppConfig se llama `SlaAppConfig` para no confundirse con el modelo `SlaConfig`. El arranque del hilo scheduler se agrega en la Task 6.)
...
En `backend/sla/apps.py` (clase `SlaAppConfig`), ampliar `ready()`:
```python
    def ready(self):
        from . import signals  # noqa: F401
        import os
