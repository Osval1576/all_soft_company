...
- **Config por tenant**: modelo `OrgAiSettings` (opt-in + rate limits + presupuesto mensual) ✅ construido (#19, #20). Gateado por `billing.effective_plan`. *Pendiente: idioma y tono.*
- **Guardrails**: rate limiting por tenant ✅ — tope de acciones de IA autenticadas por usuario/minuto (429) + tope horario de deflección en canales públicos (widget/WhatsApp/DM), que degrada a humano sin gastar IA. **Presupuesto mensual** ✅ — metering de costo por llamada (`AiUsage` + tabla de precios por proveedor/modelo); al superar `monthly_budget_usd`, `ai_enabled()` apaga la IA de la org hasta el mes siguiente. Fallback si la IA falla (degradación elegante — el producto sigue funcionando sin IA).
...
✅ **Guardrails Fase 0**: `OrgAiSettings` (opt-in por org) + **rate limiting** por tenant — usuario/minuto en las vistas autenticadas (429) y org/hora en la deflección de canales públicos (widget/WhatsApp/DM) para frenar costo/abuso (#19). **Presupuesto mensual** con metering de costo (`AiUsage` + precios por proveedor/modelo): al superar el tope, la IA de la org se apaga hasta el mes siguiente (#20).
