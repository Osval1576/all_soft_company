...
class MessengerWebhookView(APIView):
    """Webhook de Meta Messaging (Messenger + Instagram DM).

    GET  -> verificación (hub.challenge, con MESSENGER_VERIFY_TOKEN).
    POST -> mensajes entrantes (page/instagram): se procesan y, si la KB los
            resuelve, se responde automáticamente por la Send API.
    """
    permission_classes = [AllowAny]
    authentication_classes = []
    ...
    def post(self, request):
        if not messenger.verify_signature(request.body,
                                          request.headers.get("X-Hub-Signature-256")):
            return Response({"detail": "firma inválida"}, status=403)
        try:
            payload = json.loads(request.body or b"{}")
